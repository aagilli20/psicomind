...
$tpl = new tpleng;
$tpl->set_file('listado_profesionales','listado_profesionales.tpl');
...
if(isset($_REQUEST['filtrar'])){
	if(!(empty($_REQUEST["matricula"])) || ($_REQUEST["matricula"]!="Número de Matrícula")) $filtro = true;
...
if(isset($_REQUEST['filtrar']) && $filtro){
	...
	$tpl->set_var('matricula', $_REQUEST["matricula"]);
	$tpl->set_var('documento', $_REQUEST["documento"]);
	$tpl->set_var('apellido', $_REQUEST["apellido"]);
	// consulta base del listado de personas
	$sql = "SELECT p.nro_documento,p.nombre_persona,p.apellido_persona,p.sexo_id_sexo,p.tipo_documento_id_tipo_documento,pr.matricula,pr.domicilio_profesional 
	...
	if(is_alphanumeric($_REQUEST["matricula"], $min_length = 1, $max_length = 14) && ($_REQUEST["matricula"]!="Número de Matrícula")){
		$nro_mat = "%".$_REQUEST["matricula"]."%";
		$sql = $sql." AND pr.matricula LIKE '$nro_mat'";
		$sql_count = $sql_count." AND pr.matricula LIKE '$nro_mat'";
	}
	...
	if($db_cant["Count(*)"] == 0){
		...
		foreach($db_persona as $persona){
			...
			'nombre' => $persona['nombre_persona'], 'id_persona' => $persona['matricula'], 
			'domicilio_prof' => $persona['domicilio_profesional'], 'id_persona2' => base64_encode($persona['matricula']),'disabled' => ''));
		}
	}
	// cargamos el array en el bloque_persona de la plantilla listado_profesionales.tpl
	...
	// cantidad de datos
	$sql = "SELECT Count(*) FROM profesional WHERE persona_nro_documento!='root' AND activo_profesional='1'";
	...
	if(isset($_GET["pagina"])){
		...
		if($pagina > 1){
			$pag_prev = $pagina - 1;
			$tpl->set_var('enlace_pagina_prev', "listado_profesionales.php?pagina=$pag_prev");
		...
		if($ini_prox > $cantidad){
			...
			$pag_prox = $pagina + 1;
			$tpl->set_var('enlace_pagina_prox', "listado_profesionales.php?pagina=$pag_prox");
...
$tpl->parse('listado_profesionales');
