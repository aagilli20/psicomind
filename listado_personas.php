...
$tpl = new tpleng;
$tpl->set_file('listado_personas','listado_personas.tpl');
...
if(isset($_REQUEST['filtrar']) && $filtro){
	...
	if($db_cant["Count(*)"] == 0){
		...
		foreach($db_persona as $persona){
			...
			'nombre' => $persona['nombre_persona'], 'id_persona' => $id_persona, 'id_persona2' => base64_encode($id_persona),'disabled' => ''));
		}
	}
	// cargamos el array en el bloque_persona de la plantilla listado_personas
	...
	if(isset($_GET["pagina"])){
		...
		if($pagina > 1){
			$pag_prev = $pagina - 1;
			$tpl->set_var('enlace_pagina_prev', "listado_personas.php?pagina=$pag_prev");
		...
		if($ini_prox > $cantidad){
			...
			$pag_prox = $pagina + 1;
			$tpl->set_var('enlace_pagina_prox', "listado_personas.php?pagina=$pag_prox");
	...
	// cargamos el array en el bloque_persona de la plantilla listado_personas.tpl
	$tpl->set_loop('bloque_persona', $lista_persona);
	// fin lista persona
}
