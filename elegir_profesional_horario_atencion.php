...
$tpl = new tpleng;
$tpl->set_file('epha','elegir_profesional_horario_atencion.tpl');
...
if(isset($_REQUEST['filtrar']) && $filtro){
	...
	$sql_count = "SELECT Count(*) FROM persona p, profesional pr
					...
					AND p.sexo_id_sexo=pr.persona_sexo_id_sexo
					AND p.tipo_documento_id_tipo_documento=pr.persona_tipo_documento_id_tipo_documento";
	// agregamos los filtros a las consultas
	...
	// cargamos el array en el bloque_persona de la plantilla elegir_profesional_horario_atencion.tpl
	...
	if(isset($_GET["pagina"])){
		...
		if($pagina > 1){
			$pag_prev = $pagina - 1;
			$tpl->set_var('enlace_pagina_prev', "elegir_profesional_horario_atencion.php?pagina=$pag_prev");
		...
		if($ini_prox > $cantidad){
			...
			$pag_prox = $pagina + 1;
			$tpl->set_var('enlace_pagina_prox', "elegir_profesional_horario_atencion.php?pagina=$pag_prox");
