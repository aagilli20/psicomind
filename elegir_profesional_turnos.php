...
$tpl = new tpleng;
$tpl->set_file('elegir_profesional_turnos','elegir_profesional_turnos.tpl');
...
if(isset($_REQUEST['filtrar']) && $filtro){
	...
	if(isset($_GET["pagina"])){
		...
		if($pagina > 1){
			$pag_prev = $pagina - 1;
			$tpl->set_var('enlace_pagina_prev', "elegir_profesional_turnos.php?pagina=$pag_prev");
		...
		if($ini_prox > $cantidad){
			...
			$pag_prox = $pagina + 1;
			$tpl->set_var('enlace_pagina_prox', "elegir_profesional_turnos.php?pagina=$pag_prox");
...
$tpl->set_var('fecha', "DD/MM/AAAA");
// cargamos el menu principal
$tpl->set_var('menu', getMenu());
// parseamos la plantilla
$tpl->parse('elegir_profesional_turnos');
