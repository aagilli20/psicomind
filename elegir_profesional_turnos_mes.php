...
$tpl = new tpleng;
$tpl->set_file('elegir_profesional_turnos_mes','elegir_profesional_turnos_mes.tpl');
...
if(isset($_REQUEST['filtrar']) && $filtro){
	...
	// cargamos el array en el bloque persona de la plantilla elegir_profesional_especialidad.tpl
	...
	if(isset($_GET["pagina"])){
		...
		if($pagina > 1){
			$pag_prev = $pagina - 1;
			$tpl->set_var('enlace_pagina_prev', "elegir_profesional_turnos_mes.php?pagina=$pag_prev");
		...
		if($ini_prox > $cantidad){
			...
			$pag_prox = $pagina + 1;
			$tpl->set_var('enlace_pagina_prox', "elegir_profesional_turnos_mes.php?pagina=$pag_prox");
	...
	// cargamos el array en el bloque_persona de la plantilla elegir_profesional_especialidad.tpl
	$tpl->set_loop('bloque_persona', $lista_persona);
	// fin lista persona
}
// creamos array para almacenar los años
$lista_anios = array();
// forzamos a que el primer valor sea seleccione el año
array_push($lista_anios, array('id' => "0", 'valor' => "Seleccione el año"));
$anio_ini = 2001;
while($anio_ini <= date('Y')){
	array_push($lista_anios, array('id' => "$anio_ini", 'valor' => "$anio_ini"));
	$anio_ini++;
}
$tpl->set_loop('bloque_anio', $lista_anios);
// cargamos el menu principal
$tpl->set_var('menu', getMenu());
// parseamos la plantilla
$tpl->parse('elegir_profesional_turnos_mes');
