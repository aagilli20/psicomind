...
$tpl = new tpleng;
$tpl->set_file('listado_pacientes','listado_pacientes.tpl');
...
if(isset($_REQUEST['filtrar'])){
	if(!(empty($_REQUEST["codigo"])) || ($_REQUEST["codigo"]!="Código")) $filtro = true;
...
if(isset($_REQUEST['filtrar']) && $filtro){
	...
	$tpl->set_var('codigo', $_REQUEST["codigo"]);
	$tpl->set_var('documento', $_REQUEST["documento"]);
	$tpl->set_var('apellido', $_REQUEST["apellido"]);
	// consulta base del listado de personas
	$sql = "SELECT p.nro_documento,p.nombre_persona,p.apellido_persona,p.domicilio_persona,p.sexo_id_sexo,p.tipo_documento_id_tipo_documento,
				pa.id_paciente FROM persona p, paciente pa
	...
	if(is_alphanumeric($_REQUEST["codigo"], $min_length = 1, $max_length = 16) && ($_REQUEST["codigo"]!="Código")){
		$codigo = "%".$_REQUEST["codigo"]."%";
		$sql = $sql." AND pa.id_paciente LIKE '$codigo'";
		$sql_count = $sql_count." AND pa.id_paciente LIKE '$codigo'";
	}
	...
	if($db_cant["Count(*)"] == 0){
		...
		foreach($db_persona as $persona){
			...
			'nombre' => $persona['nombre_persona'], 'domicilio' => $persona['domicilio_persona'], 'id_persona' => $persona['id_paciente'], 
			'id_persona2' => base64_encode($persona['id_paciente']), 'disabled' => ''));
		}
	}
	// cargamos el array en el bloque_persona de la plantilla listado_pacientes.tpl
	...
	if(isset($_GET["pagina"])){
		...
		if($pagina > 1){
			$pag_prev = $pagina - 1;
			$tpl->set_var('enlace_pagina_prev', "listado_pacientes.php?pagina=$pag_prev");
		...
		if($ini_prox > $cantidad){
			...
			$pag_prox = $pagina + 1;
			$tpl->set_var('enlace_pagina_prox', "listado_pacientes.php?pagina=$pag_prox");
	...
	$sql = "SELECT p.nro_documento,p.nombre_persona,p.apellido_persona,p.domicilio_persona,pa.id_paciente,
				p.sexo_id_sexo,p.tipo_documento_id_tipo_documento,pa.id_paciente FROM persona p, paciente pa
				WHERE p.nro_documento!='root'
				AND pa.activo_paciente='1'
				AND p.nro_documento=pa.persona_nro_documento
				AND p.sexo_id_sexo=pa.persona_sexo_id_sexo
