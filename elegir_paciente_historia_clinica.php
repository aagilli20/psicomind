...
$tpl = new tpleng;
$tpl->set_file('elegir_paciente_historia_clinica','elegir_paciente_historia_clinica.tpl');
// inicializamos contador para la paginacion
$cantidad = 0;
...
if(isset($_REQUEST['filtrar']) && $filtro){
	...
	if($db_cant["Count(*)"] == 0){
		...
		// cargamos los resultados de la consutla en el array
	...
	// cargamos el array en el bloque_persona de la plantilla elegir_paciente_historia_clinica.tpl
	...
	if(isset($_GET["pagina"])){
		...
		if($pagina > 1){
			$pag_prev = $pagina - 1;
			$tpl->set_var('enlace_pagina_prev', "elegir_paciente_historia_clinica.php?pagina=$pag_prev");
		...
		if($ini_prox > $cantidad){
			...
			$pag_prox = $pagina + 1;
			$tpl->set_var('enlace_pagina_prox', "elegir_paciente_historia_clinica.php?pagina=$pag_prox");
...
$tpl->parse('elegir_paciente_historia_clinica');
