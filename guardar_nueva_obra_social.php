...
if(isset($_REQUEST['guardar'])){
	...
	if(!$formato_datos || !$datos_obligatorios){
		...
		$sql = "INSERT INTO obra_social(id_obra_social,rnemp,nombre,telefono,direccion,localidad,provincia,cp) 
				  VALUES ('NULL', '$rnemp', '$nombre', '$telefono', '$direccion', '$localidad', '$provincia', '$codigo_postal');";
		// inicializamos la plantilla
		$tpl = new tpleng;
		$tpl->set_file('info', 'info.tpl');
		...
		if($conexion->Execute($sql)){
			...
			$tpl->set_var('mensaje', "La obra social fue creada correctamente");
			$tpl->set_var('html_adicional', "");
			...
			// notificar error
			$tpl->set_var('mensaje', "Error al guardar la obra social en la base de datos, intentelo nuevamente");
			$tpl->set_var('html_adicional', "");
			$tpl->set_var('enlace_std', "nueva_obra_social.php");
			$tpl->set_var('mensaje_std', "Volver a Intentarlo");
			// parseamos la plantilla
