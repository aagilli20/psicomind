...
$tpl = new tpleng;
$tpl->set_file('epm','elegir_paciente_minusvalia.tpl');
...
if(isset($_REQUEST['filtrar']) && $filtro){
	...
	// cargamos el array en el bloque_persona de la plantilla elegir_paciente_minusvalia.tpl
	...
	if(isset($_GET["pagina"])){
		...
		if($pagina > 1){
			$pag_prev = $pagina - 1;
			$tpl->set_var('enlace_pagina_prev', "elegir_paciente_minusvalia.php?pagina=$pag_prev");
		...
		if($ini_prox > $cantidad){
			...
			$pag_prox = $pagina + 1;
			$tpl->set_var('enlace_pagina_prox', "elegir_paciente_minusvalia.php?pagina=$pag_prox");
