...
$tpl = new tpleng;
$tpl->set_file('epct','elegir_profesional_consignar_turno.tpl');
...
if(isset($_REQUEST['filtrar']) && $filtro){
	...
	// cargamos el array en el bloque_persona de la plantilla elegir_profesional_consignar_turno.tpl
	...
	// verificamos que se haya recibido el ID de persona o el id del paciente
	if(empty($_REQUEST["id_persona"]) && empty($_GET["id"])){
		// falta un dato
	...
	if(! empty($_GET["id"])){
		// se recibió como parametro el id del paciente
		// decodificamos el id del paciente
		$id_paciente = base64_decode($_GET["id"]);
		// verificamos la existencia del paciente
		$cant = $conexion->GetRow("SELECT Count(*) FROM paciente WHERE id_paciente='$id_paciente'");
		if($cant["Count(*)"] == 0) {
			// si no coincide con ningún paciente de la db, informamos el error
			header("location:info.php?id=4");
			// forzamos la detención del script
			die();
		}
	}
	// si llego hasta aca es porque no hay faltantes en los datos
	...
	if(isset($_GET["pagina"])){
		...
		if($pagina > 1){
			$pag_prev = $pagina - 1;
			$tpl->set_var('enlace_pagina_prev', "elegir_profesional_consignar_turno.php?pagina=$pag_prev");
		...
		if($ini_prox > $cantidad){
			...
			$pag_prox = $pagina + 1;
			$tpl->set_var('enlace_pagina_prox', "elegir_profesional_consignar_turno.php?pagina=$pag_prox");
	...
	// cargamos el array en la plantilla elegir_profesional_consignar_turno.tpl
	$tpl->set_loop('bloque_persona', $lista_persona);
	// fin listado de profesionales
	// verificamos si el acceso fue por seleccionar o por volver
	if(isset($_REQUEST["seleccionar"])){
		// fue por seleccionar, seteamos el id paciente por POST
		$tpl->set_var("id_paciente", $_REQUEST["id_persona"]);
	} else {
		// fue por volver, seteamos el id paciente por GET y lo decodificamos
		$tpl->set_var("id_paciente", base64_decode($_GET["id"]));
	}
}
