...
$tpl = new tpleng;
$tpl->set_file('epct','elegir_paciente_consignar_turno.tpl');
...
if(isset($_REQUEST['filtrar']) && $filtro){
	...
	if(isset($_GET["pagina"])){
		...
		if($pagina > 1){
			$pag_prev = $pagina - 1;
			$tpl->set_var('enlace_pagina_prev', "elegir_paciente_consignar_turno.php?pagina=$pag_prev");
		...
		if($ini_prox > $cantidad){
			...
			$pag_prox = $pagina + 1;
			$tpl->set_var('enlace_pagina_prox', "elegir_paciente_consignar_turno.php?pagina=$pag_prox");
