...
$tpl = new tpleng;
$tpl->set_file('elegir_paciente_obra_social','elegir_paciente_obra_social.tpl');
...
if(isset($_REQUEST['filtrar']) && $filtro){
	...
	if(isset($_GET["pagina"])){
		...
		if($pagina > 1){
			$pag_prev = $pagina - 1;
			$tpl->set_var('enlace_pagina_prev', "elegir_paciente_obra_social.php?pagina=$pag_prev");
		...
		if($ini_prox > $cantidad){
			...
			$pag_prox = $pagina + 1;
			$tpl->set_var('enlace_pagina_prox', "elegir_paciente_obra_social.php?pagina=$pag_prox");
...
$tpl->parse('elegir_paciente_obra_social');
